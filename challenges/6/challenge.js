...
const extractSize = htmlTemplate => {
  const heightResults = /height: (\d*)/.exec(htmlTemplate)
  const widthResults = /width: (\d*)/.exec(htmlTemplate)

  return {
    height: heightResults ? Number(heightResults[1]) : 0,
    width: widthResults ? Number(widthResults[1]) : 0,
  }
}
